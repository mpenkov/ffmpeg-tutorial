...
typedef struct VideoState {
    ...
    double
    video_clock; ///<pts of last decoded frame / predicted pts of next decoded frame
    double
    video_current_pts; ///<current displayed pts (different from video_clock if frame fifos are used)
    int64_t
    video_current_pts_time;  ///<time (av_gettime) at which we updated video_current_pts - used to have running video pts
    AVStream        *video_st;
    PacketQueue     videoq;
...
int decode_thread(void *arg) {
    ...
    for(i=0; i<pFormatCtx->nb_streams; i++) {
        printf("Scanning containder streams: TYPE: %d/%d %d", i + 1,
               pFormatCtx->nb_streams, pFormatCtx->streams[i]->codec->codec_type);
        if(pFormatCtx->streams[i]->codec->codec_type==AVMEDIA_TYPE_VIDEO &&
            ...
            printf("Something else than %d (AUDIO) or %d (VIDEO)\n", AVMEDIA_TYPE_AUDIO,
                   AVMEDIA_TYPE_VIDEO);
        }
    }
    if(audio_index >= 0) {
    ...
    if( audio_index >= 0
        ...
        printf(" New Resampler!! From Samplert: %d Hz Sample format: %s\n",
               pFormatCtx->streams[audio_index]->codec->sample_rate,
               av_get_sample_fmt_name(pFormatCtx->streams[audio_index]->codec->sample_fmt));
        printf("         To 44100 Sample format: s16\n");
    }
#endif
